...
class Amplifier:
  def __init__(self, filename, phase, in1= 0):
    self.phase = phase
    self.in1 = in1
    self.output = []
...
for code in perms:
  ...
  ampB = Amplifier(ampfile, code[1], ampA.output)
  ampC = Amplifier(ampfile, code[2], ampB.output)
  ampD = Amplifier(ampfile, code[3], ampC.output)
  ampE = Amplifier(ampfile, code[4], ampD.output)
